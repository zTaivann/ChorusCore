 ...
 *
 * <p>Reading marks the whole inbox as seen, so the reminder on login only ever counts what
 * has arrived since.
 */
public final class MailCommand extends ChorusCommand {
    ...
    public MailCommand(CommandSupport support, MailService mail) {
    ...
    protected void run(CommandSender sender, String[] args) {
        ...
        switch (action) {
            case "read" -> read(sender, args.length > 1 ? page(args[1]) : 1);
    ...
    private void read(CommandSender sender, int page) {
        ...
        mail.inbox(player.getUniqueId()).whenComplete((letters, failure) -> {
            if (failure != null) {
                messages.send(player, "error.storage");
                return;
            }
            if (letters.isEmpty()) {
                messages.send(player, "chat.mail-empty");
                return;
            }
            ...
            messages.send(player, "chat.mail-header",
